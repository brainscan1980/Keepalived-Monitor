...
type Monitor struct{cfg *config.Config;logger *slog.Logger;store *storage.Store;tracker *incidentTracker;mu sync.RWMutex;state Snapshot}
func New(cfg *config.Config,logger *slog.Logger,store *storage.Store)*Monitor{return &Monitor{cfg:cfg,logger:logger,store:store,tracker:newIncidentTracker(cfg.Notifications.NodeDown.FailuresBeforeAlert),state:Snapshot{Nodes:map[string]NodeStatus{}}}}
...
func(m *Monitor)persistIncident(i Incident){m.logger.Warn("cluster incident","type",i.Type,"scope",i.Scope,"message",i.Message);if m.store!=nil{if err:=m.store.RecordIncident(string(i.Type),i.Scope,i.Message,i.OldMaster,i.NewMaster);err!=nil{m.logger.Error("persist incident failed","error",err)}}}
...
func parseProbe(out string,s *NodeStatus){for _,line:=range strings.Split(out,"\n"){if strings.HasPrefix(line,"KEEP="){s.Keepalived=strings.TrimSpace(strings.TrimPrefix(line,"KEEP="))}else if strings.HasPrefix(line,"UP="){s.Uptime=strings.TrimSpace(strings.TrimPrefix(line,"UP="))}else if strings.HasPrefix(line,"ADDR="){var x []struct{AddrInfo []struct{Local string `json:"local"`} `json:"addr_info"`};if json.Unmarshal([]byte(strings.TrimPrefix(line,"ADDR=")),&x)==nil{for _,in:=range x{for _,a:=range in.AddrInfo{if a.Local!=""{s.Addresses=append(s.Addresses,a.Local)}}}}}}}}
