...
def _audit(target,result,details=None):
    try:_core.write_audit('failover.test',target,result,details)
    except Exception:pass

def _snapshot():
    with _core.lock:return ({k:dict(v) for k,v in (_core.cache.get('nodes') or {}).items()},[dict(v) for v in (_core.cache.get('vrrp') or [])])
def _maintenance(node):return bool((node.get('maintenance') or {}).get('active'))
def _ready(node):return bool(node.get('online') and node.get('keepalived')=='active' and not _maintenance(node))

def _instance_preflight(instance,nodes):
    name=instance.get('name');master=instance.get('master');members=list(instance.get('nodes') or []);result={'name':name,'vip':instance.get('vip'),'master':master,'members':members,'ready_backups':[],'blocked_reasons':[],'allowed':False}
    if not members:result['blocked_reasons'].append('Keine Nodes für diese VRRP-Instanz konfiguriert.');return result
    if master in {None,'NONE','MULTIPLE'}:result['blocked_reasons'].append('Es gibt aktuell keinen eindeutig ermittelten MASTER.');return result
    if master not in members:result['blocked_reasons'].append('Der aktuelle MASTER gehört nicht zur konfigurierten VRRP-Instanz.');return result
...
def _wait_failover(master,affected):
    ...
    while time.monotonic()-started<FAILOVER_TIMEOUT:
        ...
        for expected in affected:
            current=by_name.get(expected['name']) or {};new_master=current.get('master');healthy=bool(current.get('healthy'));moved=healthy and new_master not in {None,'NONE','MULTIPLE',master} and new_master in expected['ready_backups'];latest.append({'name':expected['name'],'vip':expected.get('vip'),'old_master':master,'new_master':new_master,'healthy':healthy,'moved':moved});ok=ok and moved
...
def _wait_cluster_recovery(affected):
    ...
    while time.monotonic()-started<RECOVERY_TIMEOUT:
        ...
        for expected in affected:
            current=by_name.get(expected['name']) or {};master=current.get('master');healthy=bool(current.get('healthy'));valid=healthy and master not in {None,'NONE','MULTIPLE'};latest.append({'name':expected['name'],'vip':expected.get('vip'),'master':master,'healthy':healthy,'ok':valid});ok=ok and valid
        if ok:return True,round(time.monotonic()-started,1),latest
        time.sleep(POLL_INTERVAL)
    return False,round(time.monotonic()-started,1),latest

def _execute(master,affected):
    result={'ok':False,'master':master,'affected_count':len(affected),'instances':[{'name':x['name'],'vip':x.get('vip'),'old_master':master,'ready_backups':x['ready_backups']} for x in affected],'steps':[],'recovery':{'attempted':False,'service_active':False,'cluster_healthy':False}};stopped=False
    try:
        ok,error=_service(master,'stop')
        if not ok:result['steps'].append({'step':'stop_master','ok':False,'message':error});result['error']='Keepalived auf dem MASTER konnte nicht gestoppt werden.';return result
        stopped=True;result['steps'].append({'step':'stop_master','ok':True,'message':f'Keepalived auf {master} gestoppt.'});moved,seconds,instances=_wait_failover(master,affected);result['failover']={'ok':moved,'seconds':seconds,'instances':instances};result['steps'].append({'step':'verify_failover','ok':moved,'message':f'Failover nach {seconds} s verifiziert.' if moved else f'Failover konnte innerhalb von {FAILOVER_TIMEOUT} s nicht vollständig verifiziert werden.'})
        if not moved:result['error']='Nicht alle betroffenen VRRP-Instanzen wurden erfolgreich von einem Backup übernommen.'
    except Exception as e:result['error']=f'Unerwarteter Fehler während des Failover-Tests: {e}'
    finally:
        if stopped:
            result['recovery']['attempted']=True;started_ok,start_error=_service(master,'start');result['steps'].append({'step':'start_master','ok':started_ok,'message':f'Keepalived auf {master} wieder gestartet.' if started_ok else start_error})
            if started_ok:
                active,service_seconds=_wait_service_active(master);result['recovery']['service_active']=active;result['recovery']['service_seconds']=service_seconds;result['steps'].append({'step':'verify_service','ok':active,'message':f'Keepalived auf {master} ist wieder aktiv.' if active else f'Keepalived auf {master} wurde innerhalb von {RECOVERY_TIMEOUT} s nicht wieder als aktiv erkannt.'});cluster_ok,recovery_seconds,recovery_instances=_wait_cluster_recovery(affected);result['recovery']['cluster_healthy']=cluster_ok;result['recovery']['seconds']=recovery_seconds;result['recovery']['instances']=recovery_instances;result['steps'].append({'step':'verify_cluster','ok':cluster_ok,'message':f'Cluster nach {recovery_seconds} s wieder gesund.' if cluster_ok else f'Cluster-Erholung innerhalb von {RECOVERY_TIMEOUT} s nicht vollständig verifiziert.'})
            else:result['recovery']['error']='Failsafe konnte Keepalived auf dem ursprünglichen MASTER nicht wieder starten.'
        _poll()
...
def api_run():
    ...
    try:
        ...
        if not allowed:_audit('cluster','blocked',{'reason':'Keine VRRP-Instanz sicher testbar.','preflight':pre['summary']});return jsonify({'ok':False,'error':'Aktuell ist keine VRRP-Instanz sicher testbar.','preflight':pre}),409
        if pre['summary']['blocked']:_audit('cluster','blocked',{'reason':'Sicherheits-Preflight blockiert.','preflight':pre['summary']});return jsonify({'ok':False,'error':'Failover-Test blockiert: Nicht alle VRRP-Instanzen erfüllen den Sicherheits-Preflight.','preflight':pre}),409
        masters=pre['summary']['masters']
        if len(masters)!=1:_audit('cluster','blocked',{'reason':'Kein eindeutiger gemeinsamer MASTER.','preflight':pre['summary']});return jsonify({'ok':False,'error':'Automatischer Test erfordert aktuell genau einen gemeinsamen MASTER für alle testbaren VRRP-Instanzen.','preflight':pre}),409
        master=masters[0];affected=[x for x in allowed if x['master']==master];expected=request.headers.get('X-Confirm-Failover-Master','')
        if expected!=master:_audit(master,'blocked',{'reason':'MASTER-Bestätigung fehlt oder ist veraltet.','expected':expected});return jsonify({'ok':False,'error':f'Bestätigung des betroffenen MASTER {master} fehlt oder ist nicht mehr aktuell.','preflight':pre}),409
        result=_execute(master,affected);_audit(master,'success' if result.get('ok') else 'failed',{'instances':[x['name'] for x in affected],'failover':result.get('failover'),'recovery':result.get('recovery'),'message':result.get('message'),'error':result.get('error','')});return jsonify(result),200
