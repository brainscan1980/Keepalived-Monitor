...
async function refreshAfterConfigChange(){try{current=await (await fetch('/api/settings',{cache:'no-store'})).json();ids.forEach(id=>{const e=document.querySelector('#'+id);if(!e)return;if(e.type==='checkbox')e.checked=!!current[id];else e.value=current[id]??''});summary(current)}catch(e){}await loadBackups();await loadFailoverPreflight()}
...
function failoverInstanceLine(i){const backups=(i.ready_backups||[]).join(', ')||'keiner';return `${i.allowed?'✓':'✕'} ${i.name} (${i.vip||'–'}) · MASTER ${i.master||'–'} · Backup: ${backups}${i.blocked_reasons?.length?` · ${i.blocked_reasons.join(' ')}`:''}`}
...
document.querySelector('#telegramTestBtn')?.addEventListener('click',async()=>{
    ...
    b.disabled=true;
    rEl.textContent='Telegram-Testnachricht wird gesendet…';
    rEl.className='action-result';

    try{
        const r=await fetch('/api/notifications/telegram/test',{
            method:'POST',
            headers:{
                'X-CSRF-Token':CSRF_TOKEN
            }
        ...
        rEl.textContent=j.ok
            ? '✓ Telegram-Testnachricht erfolgreich gesendet.'
            : `✕ ${j.error||'Telegram-Test fehlgeschlagen'}`;

        rEl.className=`action-result ${j.ok?'good':'bad'}`;

        if(j.ok){
            await load();
        }
    }catch(e){
        rEl.textContent=`✕ ${e.message||'Telegram-Test fehlgeschlagen'}`;
        rEl.className='action-result bad';
    }finally{
        b.disabled=false;
    }
});
