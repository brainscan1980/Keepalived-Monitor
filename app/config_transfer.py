...
def _diff(old,new):
    old_nodes={x.get('name'):x for x in old.get('nodes',[]) if isinstance(x,dict)};new_nodes={x.get('name'):x for x in new.get('nodes',[]) if isinstance(x,dict)};old_v={x.get('name'):x for x in old.get('vrrp',[]) if isinstance(x,dict)};new_v={x.get('name'):x for x in new.get('vrrp',[]) if isinstance(x,dict)}
...
def _backup_current():
    ...
    os.chmod(backup,0o600);return backup
def _atomic_write(data):
    path=_core.CONFIG_FILE;directory=os.path.dirname(path);os.makedirs(directory,exist_ok=True);fd,tmp=tempfile.mkstemp(prefix='.config-write-',suffix='.yml',dir=directory)
    try:
...
def export_config():
    ...
    data=copy.deepcopy(_current());payload=yaml.safe_dump(data,allow_unicode=True,sort_keys=False).encode('utf-8');name=f'keepalived-monitor-config-{datetime.now().strftime("%Y%m%d-%H%M%S")}.yml';return send_file(io.BytesIO(payload),mimetype='application/x-yaml',as_attachment=True,download_name=name)
...
def list_backups():
    ...
    for path in glob.glob(os.path.join(_config_dir(),'config.yml.backup-*')):
        ...
        try:_backup_path(name);st=os.stat(path);data,warnings=_read_config(path);items.append({'name':name,'created':datetime.fromtimestamp(st.st_mtime).isoformat(timespec='seconds'),'size':st.st_size,'valid':True,'summary':_summary(data),'changes':_diff(_current(),data),'warnings':warnings})
    ...
    items.sort(key=lambda x:x.get('created') or '',reverse=True);return jsonify({'ok':True,'backups':items})
...
def restore_backup(name):
    ...
    try:
        source=_backup_path(name);data,warnings=_read_config(source);safety_backup=_backup_current();_atomic_write(data);details={'backup':name,'safety_backup':os.path.basename(safety_backup) if safety_backup else None,'summary':_summary(data),'warnings':warnings}
        try:_core.poll()
        except Exception as e:_audit('config.backup.restore',name,'warning',{**details,'warning':str(e)});return jsonify({'ok':True,'warning':f'Backup wiederhergestellt, Status-Aktualisierung fehlgeschlagen: {e}','safety_backup':safety_backup,'summary':_summary(data),'warnings':warnings})
        _audit('config.backup.restore',name,'success',details);return jsonify({'ok':True,'message':'Backup erfolgreich wiederhergestellt und neu geladen.','safety_backup':safety_backup,'summary':_summary(data),'warnings':warnings})
    except ValueError as e:_audit('config.backup.restore',name,'failed',{'error':str(e)});return jsonify({'ok':False,'error':str(e)}),400
    except Exception as e:_audit('config.backup.restore',name,'failed',{'error':str(e)});return jsonify({'ok':False,'error':f'Wiederherstellung fehlgeschlagen: {e}'}),500
...
def delete_backup(name):
    ...
    try:path=_backup_path(name);os.unlink(path);_audit('config.backup.delete',name,'success');return jsonify({'ok':True,'message':'Backup gelöscht.'})
    except ValueError as e:_audit('config.backup.delete',name,'failed',{'error':str(e)});return jsonify({'ok':False,'error':str(e)}),400
    except Exception as e:_audit('config.backup.delete',name,'failed',{'error':str(e)});return jsonify({'ok':False,'error':f'Backup konnte nicht gelöscht werden: {e}'}),500
...
def apply_import():
    ...
    try:
        ...
        if errors:_audit('config.import','config.yml','failed',{'errors':errors,'warnings':warnings});return jsonify({'ok':False,'error':'Konfiguration ist ungültig.','errors':errors,'warnings':warnings}),400
        changes=_diff(_current(),data);backup=_backup_current();_atomic_write(data);details={'backup':os.path.basename(backup) if backup else None,'summary':_summary(data),'changes':changes,'warnings':warnings}
        try:_core.poll()
        except Exception as e:_audit('config.import','config.yml','warning',{**details,'warning':str(e)});return jsonify({'ok':True,'warning':f'Konfiguration gespeichert, Status-Aktualisierung fehlgeschlagen: {e}','backup':backup,'summary':_summary(data),'warnings':warnings})
        _audit('config.import','config.yml','success',details);return jsonify({'ok':True,'message':'Konfiguration erfolgreich importiert und neu geladen.','backup':backup,'summary':_summary(data),'warnings':warnings})
    except ValueError as e:_audit('config.import','config.yml','failed',{'error':str(e)});return jsonify({'ok':False,'error':str(e)}),400
    except Exception as e:_audit('config.import','config.yml','failed',{'error':str(e)});return jsonify({'ok':False,'error':f'Import fehlgeschlagen: {e}'}),500
